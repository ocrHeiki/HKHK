...
<html lang="en">
    <head>
        <title>Harjutused 5 ja edasi</title>
        <!-- Required meta tags -->
        <meta charset="utf-8" />
        <meta
            name="viewport"
    ...
    <div class="container">
        <h1>Massiivid</h1>
    <?php
    $nimed = array("Mari", "Kati", "Mai", "Hai", "Elevant");
...
if (count($cars)==count($vins)) {
    echo "Autod (".count($cars).") ja VINid (".count($vins).") klapivad";
} else {
    echo "Autod (".count($cars).") ja VINid (".count($vins).") ei klapi";
}
echo "<br>---------------------------------<br>";
...
echo "Toyota: $toyota<br>Audi: $audi";
echo "<br>---------------------------------<br>";
foreach ($vins as $vin) {
    if (strlen ($vin)< 17) {
        echo "$vin<br>";
...
for ($i=1; $i <= 100; $i++) { // tsükkel mille läbi käime
    if ($i%10==0) { // %=jäägiga jagamine
        echo $i.".<br>";
    } else {
        echo $i.". ";
    }
...
//head mõtted
function headmotted(){
    $alus = array("Mari", "Kati", "Hai");
    $oeldis = array("kamandab", "poob", "laseb");
    $sihitis = array("last", "äia", "meest");
